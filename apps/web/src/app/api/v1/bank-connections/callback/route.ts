 ...
 */
import { completeBankConnection, toAppError } from "@sagolik/core";
...
export async function GET(request: Request) {
  ...
  if (!actor) return NextResponse.redirect(new URL(`/sign-in?next=${encodeURIComponent(url.pathname + url.search)}`, url.origin));
  ...
  try {
    ...
    return NextResponse.redirect(new URL(`/app/settings/banks?bank_error=${encodeURIComponent(err.userMessage)}`, url.origin));
  }
}
