...
const REC_LABEL = { not_ready: "Not ready", ready_for_recording: "Ready for recording", submitted_for_recording: "Submitted to the registry", recorded: "Recorded", rejected: "Rejected by the registry" } as const;
...
export default async function ClosingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
      <div className="space-y-6">
        {r?.status === "recorded" ? (
          <Alert tone="done" title="Ownership has transferred">
            {j.recording.registry} confirmed the recording{r.recordingReference ? ` (reference ${r.recordingReference})` : ""}.{" "}
            {record ? (
        ...
        <Card>
          <CardHeader title="Ready to record?" description="The deed can be submitted for recording only when every requirement below is met." />
          <CardBody>
      ...
      <div className="space-y-6">
        <Card>
          <CardHeader title="Recording" action={r ? <StatusBadge tone={r.status === "recorded" ? "done" : r.status === "rejected" ? "blocked" : r.status === "not_ready" ? "neutral" : "progress"}>{REC_LABEL[r.status]}</StatusBadge> : null} />
          <CardBody className="space-y-4">
            <DefinitionList
              items={[
                { term: "Registry", value: r?.registry ?? j.recording.registry },
            ...
            {r?.status === "submitted_for_recording" && canConfirm ? (
              <ActionForm action={confirmRecordingAction} className="space-y-3 rounded-lg border border-line p-4">
                ...
                <p className="flex items-center gap-2 text-sm font-medium text-ink">
                  <BadgeCheck className="h-4 w-4 text-teal-600" aria-hidden /> Confirm the registry recorded the deed
                </p>
                <Field label="Registry" htmlFor="registry">
                  <Input id="registry" name="registry" defaultValue={r.registry} required />
                </Field>
                <Field label="Recording reference (instrument / document number)" htmlFor="recordingReference">
                  <Input id="recordingReference" name="recordingReference" required minLength={3} />
                </Field>
                <Field label="Recorded at" htmlFor="recordedAt">
                ...
                <label className="flex items-start gap-2 text-[13px] text-ink-2">
                  <input type="checkbox" name="attestation" required className="mt-0.5 accent-navy-800" />I attest that {r.registry} has confirmed this recording. This is recorded under my name.
                </label>
                <SubmitButton>Confirm recording</SubmitButton>
              </ActionForm>
            ) : null}
            {r?.status !== "recorded" ? <p className="text-[12px] text-ink-3">Ownership is only marked transferred after the registry confirms the recording — never by a click.</p> : null}
          </CardBody>
        </Card>
